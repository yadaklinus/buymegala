...
export default function DashboardPage() {
    ...
    useEffect(()=>{
      ...
      if(status === "authenticated"){
        main()
        main2((session?.user?.email)?.split("@")[0])
      }
    },[status])
   
                       
    const text = `http://localhost:8000/@${(session?.user?.email)?.split("@")[0]}`
    ...
    return (
        <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="bg-white p-6 rounded-lg shadow-md flex items-center space-x-4"><div className="bg-green-100 p-3 rounded-full"><User className="h-6 w-6 text-green-600" /></div><div><p className="text-gray-500">Total Supporters</p><p className="text-2xl font-bold text-gray-800">{supporters.length}</p></div></div>
